...
namespace Lottery.CommandHandlers
{
    public class OrderCommandHandler : ICommandHandler<AddOrderRecordCommand>
    {
        ...
        {
            context.Add(new OrderRecord(command.AggregateRootId, command.SalesOrderNo,command.GoodsId, command.AuthRankId, command.LotteryId, command.OrderSourceType, command.Count,
